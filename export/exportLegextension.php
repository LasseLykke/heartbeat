...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_name'])) {
  ...
  <head>
    ...
    <title>H E A R T B E A T || LEGEXTENSION STATS </title>
  </head>
  ...
  <div class="wrapper">
    ...
    <section class="wrapper">
      <div class="export-charts">
        <div class="chart-container">
          ...
          $sql = "SELECT DATE_FORMAT(ws.sessionDate, '%Y-%m-%d') AS date, wa.legextensionRep, wa.legextensionKilo
                            FROM woExtension AS wa
          ...
          $legextensionRepData = [];
          $legextensionKiloData = [];

          if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              $legextensionRepData[] = [
                'x' => $row['date'],
                'y' => $row['legextensionRep']
              ];
              $legextensionKiloData[] = [
                'x' => $row['date'],
                'y' => floatval ($row['legextensionKilo'])
              ];
        ...
        <script>
          ...
          const legextensionRepData = <?php echo json_encode($legextensionRepData); ?>;
          const legextensionKiloData = <?php echo json_encode($legextensionKiloData); ?>;
          ...
          <table id="statsTable">
            ...
            <tbody>
              ...
              foreach ($tableData as $row) {
                ...
                echo "<td>{$row['legextensionRep']}</td>";
                echo "<td>{$row['legextensionKilo']}</td>";
                echo "</tr>";
              }
              ?>
            </tbody>
    ...
    <script>
      ...
      const formattedDates = legextensionRepData.map((data) => {
      ...
      const workoutLineChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Reps",
              data: legextensionRepData.map((data) => data.y), // Brug kun y-værdierne (reps)
            ...
            {
              label: "Kilo",
              data: legextensionKiloData.map((data) => data.y), // Brug kun y-værdierne (kilo)
      ...
      setTimeout(function () {
        ...
        const lastIndex = legextensionRepData.length - 1;

        // Beregn scroll-position
        const scrollPosition = (totalWidth / legextensionRepData.length) * lastIndex;
