...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_name'])) {
  ...
  <head>
    ...
    <title>H E A R T B E A T || TRICEPS STATS </title>
  </head>
  ...
  <div class="wrapper">
    ...
    <section class="wrapper">
      <div class="export-charts">
        <div class="chart-container">
          ...
          $sql = "SELECT DATE_FORMAT(ws.sessionDate, '%Y-%m-%d') AS date, wa.tricepsRep, wa.tricepsKilo
                            FROM woTriceps AS wa
        ...
        <script>
          ...
          const tricepsRepData = <?php echo json_encode($tricepsRepData); ?>;
          const tricepsKiloData = <?php echo json_encode($tricepsKiloData); ?>;
          ...
          <table id="statsTable">
            ...
            <tbody>
              ...
              foreach ($tableData as $row) {
                ...
                echo "<td>{$row['tricepsRep']}</td>";
                echo "<td>{$row['tricepsKilo']}</td>";
                echo "</tr>";
              }
              ?>
            </tbody>
    ...
    <script>
      ...
      const formattedDates = tricepsRepData.map((data) => {
      ...
      const workoutLineChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Reps",
              data: tricepsRepData.map((data) => data.y), // Brug kun y-værdierne (reps)
            ...
            {
              label: "Kilo",
              data: tricepsKiloData.map((data) => data.y), // Brug kun y-værdierne (kilo)
      ...
      setTimeout(function () {
        ...
        const lastIndex = tricepsRepData.length - 1;

        // Beregn scroll-position
        const scrollPosition = (totalWidth / tricepsRepData.length) * lastIndex;
