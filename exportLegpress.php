...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_name'])) {
  ...
  <head>
    ...
    <title>H E A R T B E A T || LEGPRESS STATS </title>
  </head>
  ...
  <div class="wrapper">
    <section class="hbHeader">
      <h1 class="headerText">Legpress statestik</h1>
    </section>
    ...
    <section class="wrapper">
      <div class="export-charts">
        <div class="chart-container">
          ...
          $sql = "SELECT DATE_FORMAT(ws.sessionDate, '%Y-%m-%d') AS date, wa.legpressRep, wa.legpressKilo
                            FROM woLegpress AS wa
        ...
        <script>
          ...
          const legpressRepData = <?php echo json_encode($legpressRepData); ?>;
          const legpressKiloData = <?php echo json_encode($legpressKiloData); ?>;
          ...
          <table id="statsTable">
            ...
            <tbody>
              ...
              foreach ($tableData as $row) {
                ...
                echo "<td>{$row['legpressRep']}</td>";
                echo "<td>{$row['legpressKilo']}</td>";
                echo "</tr>";
              }
              ?>
            </tbody>
    ...
    <script>
      ...
      const formattedDates = legpressRepData.map((data) => {
      ...
      const workoutLineChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Reps",
              data: legpressRepData.map((data) => data.y), // Brug kun y-værdierne (reps)
            ...
            {
              label: "Kilo",
              data: legpressKiloData.map((data) => data.y), // Brug kun y-værdierne (kilo)
      ...
      setTimeout(function () {
        ...
        const lastIndex = legpressRepData.length - 1;

        // Beregn scroll-position
        const scrollPosition = (totalWidth / legpressRepData.length) * lastIndex;
