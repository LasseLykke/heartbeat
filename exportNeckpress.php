...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_name'])) {
  ...
  <head>
    ...
    <title>H E A R T B E A T || NECKPRESS STATS </title>
  </head>
  ...
  <div class="wrapper">
    <section class="hbHeader">
      <h1 class="headerText">Neckpress statestik</h1>
    </section>
    ...
    <section class="wrapper">
      <div class="export-charts">
        <div class="chart-container">
          ...
          $sql = "SELECT DATE_FORMAT(ws.sessionDate, '%Y-%m-%d') AS date, wa.neckpressRep, wa.neckpressKilo
                            FROM woNeck AS wa
...
const formattedDates = neckpressRepData.map((data) => {
...
const workoutLineChart = new Chart(ctx, {
  ...
  data: {
    ...
    datasets: [
      {
        label: "Reps",
        data: neckpressRepData.map((data) => data.y), // Brug kun y-værdierne (reps)
      ...
      {
        label: "Kilo",
        data: neckpressKiloData.map((data) => data.y), // Brug kun y-værdierne (kilo)
...
setTimeout(function () {
  ...
  const lastIndex = neckpressRepData.length - 1;

  // Beregn scroll-position
  const scrollPosition = (totalWidth / neckpressRepData.length) * lastIndex;
...
=======
          ...
          const neckpressRepData = <?php echo json_encode($neckpressRepData); ?>;
          const neckpressKiloData = <?php echo json_encode($neckpressKiloData); ?>;
>>>>>>> main
