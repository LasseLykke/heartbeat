...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_name'])) {
  ...
  <head>
    ...
    <title>H E A R T B E A T || PULLUPS STATS </title>
  </head>
  ...
  <div class="wrapper">
    <section class="hbHeader">
      <h1 class="headerText">Pullups statestik</h1>
    </section>
    ...
    <section class="wrapper">
      <div class="export-charts">
        <div class="chart-container">
          ...
          $sql = "SELECT DATE_FORMAT(ws.sessionDate, '%Y-%m-%d') AS date, wa.pullupsRep, wa.pullupsKilo
                            FROM woPullups AS wa
        ...
        <script>
          ...
          const pullupsRepData = <?php echo json_encode($pullupsRepData); ?>;
          const pullupsKiloData = <?php echo json_encode($pullupsKiloData); ?>;
          ...
          <table id="statsTable">
            ...
            <tbody>
              ...
              foreach ($tableData as $row) {
                ...
                echo "<td>{$row['pullupsRep']}</td>";
                echo "<td>{$row['pullupsKilo']}</td>";
                echo "</tr>";
              }
              ?>
            </tbody>
    ...
    <script>
      ...
      const formattedDates = pullupsRepData.map((data) => {
      ...
      const workoutLineChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Reps",
              data: pullupsRepData.map((data) => data.y), // Brug kun y-værdierne (reps)
            ...
            {
              label: "Kilo",
              data: pullupsKiloData.map((data) => data.y), // Brug kun y-værdierne (kilo)
      ...
      setTimeout(function () {
        ...
        const lastIndex = pullupsRepData.length - 1;

        // Beregn scroll-position
        const scrollPosition = (totalWidth / pullupsRepData.length) * lastIndex;
