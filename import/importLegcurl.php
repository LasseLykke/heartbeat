...
<body>
    ...
    <div class="wrapper">
        ...
        <section class="lastStats">
            ...
            function getLastExerciseData($conn, $woLegcurl)
            {
                $sql = "SELECT 
                legcurlRep, 
                legcurlKilo, 
            ...
            FROM 
                $woLegcurl
            JOIN 
                workoutSession ON $woLegcurl.sessionID = workoutSession.sessionID
            WHERE 
                legcurlID = (SELECT MAX(legcurlID) FROM $woLegcurl WHERE sessionID = workoutSession.sessionID)
            ORDER BY 
                sessionDate DESC
            LIMIT 1;";
            ...
            $woLegcurl = "woLegcurl"; // Skift dette til andre tabeller som "triceps", "legs" osv.
            $data = getLastExerciseData($conn, $woLegcurl);
            ...
            if ($data) {
                ...
                echo "<p>{$data['legcurlRep']} Rep | {$data['legcurlKilo']} Kilo</p>";
            } else {
                echo "<p>Ingen data fundet for $woLegcurl.</p>";
            }
            ?>
        </section>
