...
<body>
    ...
    <div class="wrapper">
        ...
        <section class="lastStats">
            ...
            function getLastExerciseData($conn, $woPullups)
            {
                $sql = "SELECT 
                pullupsRep, 
                pullupsKilo, 
            ...
            FROM 
                $woPullups
            JOIN 
                workoutSession ON $woPullups.sessionID = workoutSession.sessionID
            WHERE 
                pullupsID = (SELECT MAX(pullupsID) FROM $woPullups WHERE sessionID = workoutSession.sessionID)
            ORDER BY 
                sessionDate DESC
            LIMIT 1;";
            ...
            $woPullups = "woPullups"; // Skift dette til andre tabeller som "triceps", "legs" osv.
            $data = getLastExerciseData($conn, $woPullups);
            ...
            if ($data) {
                ...
                echo "<p>{$data['pullupsRep']} Rep | {$data['pullupsKilo']} Kilo</p>";
            } else {
                echo "<p>Ingen data fundet for $woPullups.</p>";
            }
            ?>
        </section>
